function CourseList({ courses, onCourseClick, selectedCourse }) {
    return (
        <section>
            ...
            // space-y-3 is used to add a vertical space of 0.75rem (12px) between the course cards.*/}
            <div className="space-y-3">
                ...
                // shadow-md is used to add a medium shadow to the course card when the user hovers over it.
                    ...
                    className={`w-full rounded-xl p-4 text-left transition ${selectedCourse == course.id 
                        ? "bg-blue-600 text-white shadow-md" : "bg-white hover:bg-gray-50 hover:shadow-sm"
                     }`}>
                        <p className="font-semibold"> {course.fullname} </p>
                    </button>
                ))}
            </div>
        </section>
